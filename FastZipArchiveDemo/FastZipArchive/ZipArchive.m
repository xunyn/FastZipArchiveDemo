...
-(BOOL) FastUnzipFileTo:(NSString*) path overWrite:(BOOL) overwrite
{
	BOOL success = YES;
	int ret = unzGoToFirstFile( _unzFile );
	unsigned char		buffer[4096*16] = {0};
    
    //createDirectory
    FZipCreateDirectory * cd ;
    cd = [[[FZipCreateDirectory alloc]init] autorelease];
	...
	do{
		...
		if( isDirectory )
            //xun
        {
            // use private objecte create directory
            [cd checkDirectory:fullPath zipPath:strPath];
        }
		...
		while( fp )
		{
			read=unzReadCurrentFile(_unzFile, buffer, 4096*16);
			if( read > 0 )
			{
				fwrite(buffer, read, 1, fp );
			}
		...
		{
			fclose( fp );
            //Don't change default modification time
